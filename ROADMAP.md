...
| 2 | Supabase Google OAuth, `profiles`, RLS | Sign in/out works, profile row auto-created, RLS verified | 🟡 **DB + code done, blocked** — Google provider not enabled in Supabase, so sign-in is unproven end to end |
