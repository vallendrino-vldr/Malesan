...
const STARTERS_ID: { label: string; seed: string }[] = [
  {
    label: "Kasir warung",
...
export function VibeCodingStudio({ cost = 6 }: { cost?: number }) {
  ...
  const STARTERS_EN: { label: string; seed: string }[] = [
    { label: v.starterKasir, seed: v.starterKasirSeed },
    { label: v.starterAbsensi, seed: v.starterAbsensiSeed },
    { label: v.starterKatalog, seed: v.starterKatalogSeed },
    { label: v.starterHabit, seed: v.starterHabitSeed },
    { label: v.starterSplitBill, seed: v.starterSplitBillSeed },
    { label: v.starterLes, seed: v.starterLesSeed },
  ];

  const IDEA_STARTERS = isEn ? STARTERS_EN : STARTERS_ID;
  ...
  async function ask() {
    if (idea.trim().length < 12) {
      setError(
        isEn
          ? "Please describe what you want to build in at least one full sentence."
          : "Ceritain dulu mau bikin apa, minimal satu kalimat.",
      );
      return;
    }
    setAsking(true);
    ...
    <section className="space-y-5">
      ...
      <div className="surface-card rounded-2xl border border-hairline/80 p-1 flex items-center gap-1 shadow-xs max-w-md mx-auto">
        <button
          ...
          className={`flex-1 h-8.5 flex items-center justify-center gap-1.5 rounded-xl text-xs font-semibold transition-all cursor-pointer ${
            hubMode === "learn"
        ...
        >
          ...
          <span className="font-display">{v.modeLearn}</span>
        </button>
        ...
        >
          ...
          <span className="font-display">{v.modeVibe}</span>
        </button>
      </div>
        ...
        <div className="space-y-6 animate-fadeIn">
          <header>
            ...
            <h2 className="mt-3 font-display text-2xl font-bold leading-tight tracking-display-md text-ink sm:text-3xl">
              {isEn ? "Tell us what app you want to build." : "Ceritain aplikasi yang mau lo bikin."}
            </h2>
            <p className="mt-2 max-w-2xl text-xs sm:text-sm leading-relaxed text-muted">
              {isEn
                ? "AI coding agents need comprehensive instructions. Malesan will craft the entire architecture blueprint ready to code — PRD, design system, roadmap, agent rules, database schema, and kickoff prompt."
          ...
          <div className="surface-card rounded-3xl border border-ember/35 bg-gradient-to-b from-surface-raised/80 via-surface to-obsidian p-5 sm:p-6 shadow-md">
            <label htmlFor="vibe-idea" className="eyebrow block text-ember font-bold">
              {isEn ? "App Idea & Concept" : "Ide & Konsep Aplikasi"}
            </label>
            <textarea
              id="vibe-idea"
              value={idea}
              ...
              rows={4}
              placeholder={v.vibeInputPlaceholder}
            ...
            {!idea.trim() && (
              <div className="mt-3">
                <p className="text-micro text-muted font-medium">
                  {isEn ? "Need inspiration? Start with these quick concepts:" : "Belum kepikiran? Mulai dari ide cepat ini:"}
                </p>
            ...
            <div className="mt-4 grid gap-3 border-t border-hairline/60 pt-4 sm:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  placeholder={isEn ? "Next.js + Supabase, Flutter, etc." : "Next.js + Supabase, Flutter, dll."}
              ...
              <div>
                <label htmlFor="vibe-audience" className="eyebrow block text-muted">
                  {isEn ? "Target Audience" : "Target User"}{" "}
                  <span className="normal-case tracking-normal text-muted/70">
                    ({isEn ? "optional" : "opsional"})
                  </span>
                </label>
                <input
                  ...
                  placeholder={isEn ? "Young couples, small businesses, students" : "Pasangan muda, UMKM, mahasiswa"}
                  className="mt-1.5 w-full skeu-inset rounded-xl border border-hairline bg-obsidian px-3.5 py-2.5 text-xs sm:text-sm text-ink outline-none transition-colors duration-[var(--duration-standard)] ease-heat placeholder:text-muted/60 focus:border-ember/50"
                />
              </div>
            </div>
            ...
            <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <button
                ...
                className="btn-ember inline-flex min-h-12 items-center justify-center rounded-xl px-6 font-display text-sm sm:text-base font-bold text-obsidian shadow-md transition-transform active:scale-[0.99] hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-50 cursor-pointer"
              >
                {asking
                  ? isEn
                    ? "Preparing questions..."
                  ...
                  ? isEn
                    ...
                    : "Lagi meracik blueprint..."
                  : isEn
                  ? v.vibeGenerateBtn
                  : "Males mikir. Bikinin Blueprint App →"}
              </button>
              <span className="text-xs text-muted">
                <span className="tabular font-mono font-bold text-ember">{cost}</span>{" "}
                {isEn ? "credits · 6 complete architecture documents" : "kredit · 6 dokumen arsitektur lengkap"}
              </span>
            </div>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {step && (
                      <p className="mt-0.5 font-mono text-micro text-muted">
                        {step.done} {isEn ? "of" : "dari"} {step.total} {isEn ? "documents" : "dokumen"}
                      </p>
                    )}
                  </div>
                </div>
            ...
            <section className="space-y-3">
              <div className="flex items-center justify-between px-0.5">
                ...
                <span className="text-micro font-mono text-muted">
                  {isEn ? "6 industry-standard documents" : "6 dokumen standar industri"}
                </span>
              </div>
                ...
                {VIBE_KIT_DOCS.map((d) => (
                  ...
                  >
                    <div className="flex items-baseline justify-between gap-2">
                      ...
                      <span className="font-mono text-micro text-ember bg-surface-raised px-1.5 py-0.5 rounded border border-hairline">
                        {d.file}
                      </span>
                    </div>
              ...
              <div className="surface-card rounded-2xl border border-hairline p-4 sm:p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <button
                    type="button"
                    onClick={downloadAll}
                    className="shrink-0 rounded-xl border border-hairline bg-surface px-4 py-2.5 font-display text-sm font-semibold text-ink transition-colors duration-[var(--duration-standard)] ease-heat hover:border-ember/40 hover:text-ember-lo cursor-pointer"
                  >
                    {isEn ? "Download All" : "Download semua"}
                  </button>
                </div>
              </div>
                ...
                >
                  {VIBE_KIT_DOCS.map((d) => (
                    <button
                      ...
                      className={`whitespace-nowrap rounded-full border px-3.5 py-2 font-display text-sm font-semibold transition-colors duration-[var(--duration-standard)] ease-heat cursor-pointer ${
                        active === d.key
                  ...
                  <div className="flex items-center justify-between gap-3 border-b border-hairline px-4 py-3">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="rounded-lg border border-hairline px-3 py-1.5 text-xs font-semibold text-muted transition-colors duration-[var(--duration-standard)] ease-heat hover:text-ink cursor-pointer"
                      >
                        {copied === d.key ? (isEn ? "Copied" : "Kesalin") : (isEn ? "Copy" : "Salin")}
                      </button>
                      <button
                        type="button"
