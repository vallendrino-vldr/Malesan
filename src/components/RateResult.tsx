  ...
  if (done) {
    return (
      <p className={`text-micro leading-relaxed text-success ${className}`}>
        {done === "good"
          ? isEn
            ? "Noted. We'll generate more results like this."
            : "Noted. Yang model kayak gini bakal lebih sering keluar."
          : isEn
          ? "Noted. We'll avoid this pattern next time."
  ...
  return (
    <div className={className}>
      <div className="flex items-center gap-2">
        <span className="text-micro text-muted">{isEn ? "Useful?" : "Kepake gak?"}</span>
        <button
          onClick={() => send("good")}
          disabled={busy}
          aria-label={isEn ? "This was useful" : "Hasilnya kepake"}
        ...
        <button
          onClick={() => send("bad")}
          disabled={busy}
          aria-label={isEn ? "Not useful" : "Hasilnya gak kepake"}
        ...
        >
          ...
          {isEn ? "Not useful" : "Enggak"}
        </button>
      </div>
      {failed && (
        <p className="mt-1 text-micro text-danger">
          {isEn ? "Failed to send. Please try again." : "Gagal kekirim. Coba lagi."}
        </p>
      )}
      <p className="mt-1.5 text-micro leading-relaxed text-muted">
        {isEn
          ? "This trains the model to align with your personal style. Scoped exclusively to your account."
          : "Ini yang bikin hasil berikutnya makin nyambung sama lo. Cuma kepake buat akun lo sendiri."}
      </p>
    </div>
  );
}
