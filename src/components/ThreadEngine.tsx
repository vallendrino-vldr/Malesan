...
export function ThreadEngine({ cost }: { cost: number }) {
  ...
  const run = async () => {
    if (!bullets.trim()) {
      setError(
        isEn
          ? "Please paste your key points first. Raw numbers are fine too."
          : "Tempel dulu poin-poinnya. Angka mentah juga gak apa-apa."
      );
      return;
    }
    setBusy(true);
    ...
    try {
      ...
      if (!res.ok) throw new Error(await readErrorBody(res, isEn ? "Failed to generate thread." : "Gagal bikin thread."));
    ...
    <div className="space-y-4">
      <section className="surface-card rounded-2xl p-5">
        <h2 className="font-display text-xl font-bold tracking-display-sm text-ink">
          {isEn ? "Thread Engine" : "Bikin Thread"}
        </h2>
        <p className="mt-2 text-sm leading-relaxed text-muted">
          {isEn
            ? "For market or tech discussions. Turn raw daily notes into an engaging thread read until the very last post."
            : "Buat yang bahas market atau tech. Lempar catatan mentah hari ini, balik jadi thread yang kebaca sampai post terakhir."}
        </p>

        <div className="mt-4 space-y-3.5">
          <div>
            <label htmlFor="thread-bullets" className="block text-sm font-semibold text-ink">
              {isEn ? "What are your key points?" : "Poin-poinnya apa aja?"}
            </label>
            <p className="mt-0.5 text-micro text-muted">
              {isEn
                ? "One point per line. Keep exact numbers — let them be used as-is."
                : "Satu baris satu poin. Angkanya jangan dibulatin — biar dipakai apa adanya."}
            </p>
            <textarea
              id="thread-bullets"
              rows={6}
              ...
              placeholder={
                isEn
                  ? "S&P 500 down 1.2% to 5,420\nForeign net outflow $840M\nBanking sector hit hardest, JPM -2.1%\n10-year yield steady at 4.25%"
                  : "IHSG turun 1,2% ke 7.180\nAsing net sell Rp 840 M\nSektor bank paling dalem, BBRI -2,1%\nRupiah 16.240 per USD"
              }
        ...
        >
          {busy
            ? isEn
              ? "Composing thread..."
              : "Lagi nyusun threadnya..."
            : isEn
              ? `Generate thread · ${cost} credits`
              : `Bikin thread · ${cost} kredit`}
        </button>
      </section>
      ...
      {busy && (
        <GenerationProgress
          moduleKey="thread"
          chars={chars}
          label={isEn ? "Composing thread" : "Lagi nyusun threadnya"}
          status={status}
        />
      )}
        ...
        <div className="space-y-3">
          {everything && (
            <div className="flex items-center justify-between rounded-xl border border-hairline bg-obsidian px-4 py-2.5">
              <p className="eyebrow text-muted">
                Thread · {posts.length + (out.closing_post ? 1 : 0) + (hook ? 1 : 0)}{" "}
                {isEn ? (posts.length + (out.closing_post ? 1 : 0) + (hook ? 1 : 0) === 1 ? "post" : "posts") : "post"}
              </p>
              <CopyBtn text={everything} label={isEn ? "Copy all" : "Salin semua"} inline />
            </div>
          )}
              ...
              {alts.length > 0 && (
                <div className="mt-3 border-t border-hairline pt-3">
                  <p className="text-micro text-muted">
                    {isEn ? "Want a different angle? Swap with this:" : "Kurang nendang? Tuker sama yang ini:"}
                  </p>
                  <div className="mt-2 space-y-1.5">
                    <AltHook
                      on={altHook === -1}
                      text={out.hook_post ?? ""}
                      label={isEn ? "Original version" : "Versi asli"}
                    ...
                    {alts.map((a, i) => (
                      <AltHook
                        ...
                        on={altHook === i}
                        text={a}
                        label={isEn ? `Alternative ${i + 1}` : `Alternatif ${i + 1}`}
          ...
          {out.closing_post && (
            <section className="surface-card rounded-2xl p-4">
              <div className="flex items-baseline justify-between gap-3">
                <p className="eyebrow text-muted">{posts.length + 2} · {isEn ? "Closing" : "Penutup"}</p>
