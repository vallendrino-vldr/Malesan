...
export async function POST(request: NextRequest) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      /** Filled once the engine reports which model actually served the run. */
...
`
        ...
        const identityRaw = identityRun.text;
        // Every document in this kit is generated by the same run, so the model
        // that answered the identity call is the model the kit was written on.
        usedModel = identityRun.modelId;
        const identity = parseAIJson<{
          project_name: string;
          one_liner: string;
        ...
        const results = await Promise.all(
          VIBE_DOC_SPECS.map(async (doc) => {
            ...
            const content = parseAIJson<{ content: string }>(raw)?.content ?? "";
            done += 1;
            send({
        ...
        const { data: genRow } = await serviceRole
          ...
          .insert({
            ...
            credits_spent: cost,
            // What actually ran, not what we intended to run. Those differ the
            // moment the Brain routes elsewhere or a fallback fires, and a
            // history row claiming Gemini when DeepSeek wrote it is how the
            // "already switched but still says Gemini" confusion starts.
        ...
        console.error("vibe kit failed", err);

        // Not charged for a failure. Exact reversal by ref, idempotent.
        await refundCredits(user.id, spendRef, "refund_vibe_kit_failed");

        // The internal message named the vendor and, on an auth failure, part of
        // a key. The user gets the product's voice and the one fact that matters.
        send({
          error: `${userFacingError(err).message} Kredit lo udah dibalikin.`,
        });
      } finally {
        controller.close();
      }
